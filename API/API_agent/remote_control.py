...
def remote_control_shell(json_module_args={'command': 'hostname'}, int_timeout=30, int_background=0):
    ...
    try:
        stdout_coding = chardet.detect(_stdout['stdout'].encode('raw_unicode_escape'))['encoding']
        stderr_coding = chardet.detect(_stdout['stderr'].encode('raw_unicode_escape'))['encoding']
        if stdout_coding != 'ascii':
            _stdout['stdout'] = _stdout['stdout'].encode('raw_unicode_escape').decode(stdout_coding)
        if stderr_coding != 'ascii':
            _stdout['stderr'] = _stdout['stderr'].encode('raw_unicode_escape').decode(stderr_coding)
    except Exception, e:
        pass
    if 'failed' in _stdout or _stdout.get('rc', 0) != 0:
        return {"success": False, "msg": _stdout}
    else:
...
def remote_control_copy(json_module_args, int_timeout=30, int_background=0):
    ...
    return {"success": False, "msg": u"原地址不合法"}
