...
var resizeWorldMapContainer = function () {
    document.getElementById('echarts_detect_network'+asset_id).style.width = (parseInt(window.innerWidth)-20)+'px';
    document.getElementById('echarts_detect_network'+asset_id).style.height = window.innerHeight+'px';
};
var color = ['#a6c84c', '#ffa022', '#FF4500'];
var series = [];
function network_detect_task_add_single_with_id(id){
    $.ajax({
        type : 'POST',
        url: "/web/network_detect/task/add_single/",
        data: {
            "id": $('#task_id'+id).val(),
            "name": $('#task_name'+id).val(),
            "desc": $('#task_desc'+id).val(),
            "ping": $('#switch_ping'+id).is(':checked'),
            "ping_detect_steps": $('#ping_detect_steps'+id).val(),
            "ping_detect_num": $('#ping_detect_num'+id).val(),
            "ping_detect_interval": $('#ping_detect_interval'+id).val(),
            "ping_detect_timeout": $('#ping_detect_timeout'+id).val(),
            "ping_detect_size": $('#ping_detect_size'+id).val(),
            "traceroute": $('#switch_traceroute'+id).is(':checked'),
            "traceroute_detect_steps": $('#traceroute_detect_steps'+id).val(),
            "traceroute_detect_num": $('#traceroute_detect_num'+id).val(),
            "traceroute_detect_timeout": $('#traceroute_detect_timeout'+id).val(),
            "curl": $('#switch_curl'+id).is(':checked'),
            "curl_detect_steps": $('#curl_detect_steps'+id).val(),
            "curl_detect_timeout_dns": $('#curl_detect_timeout_dns'+id).val(),
            "curl_detect_timeout_conn": $('#curl_detect_timeout_conn'+id).val(),
            "curl_detect_timeout_total": $('#curl_detect_timeout_total'+id).val(),
            "task_source":  $('#task_source'+id+' option:selected').val(),
            "task_target":  $('#task_target'+id+' option:selected').val(),
            "task_uri": $('#task_uri'+id).val(),
        },
...
function network_detect_task_del_single_with_id(id){
    $.ajax({
        type : 'POST',
